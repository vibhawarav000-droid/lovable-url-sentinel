...
const AuditLogs: React.FC = () => {
  ...
  const getActionBadge = (action: string) => {
    ...
    switch (type) {
      ...
      case 'invite':
        return <Badge className="bg-warning/10 text-warning border-warning/20">Invite</Badge>;
      case 'acknowledge':
        return <Badge variant="secondary">Acknowledge</Badge>;
      default:
            ...
            <TableBody>
              {paginatedLogs.map((log) => (
                <TableRow key={log.id} className="animate-fade-in">
                  ...
                  <TableCell>
                    <p className="text-sm max-w-[200px] truncate">{log.details}</p>
                  </TableCell>
                  <TableCell>
