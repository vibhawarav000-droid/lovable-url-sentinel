...
const URLPerformanceReport: React.FC = () => {
  const [selectedMonitor, setSelectedMonitor] = useState<string>(mockMonitors[0]?.id || '');
  ...
  const selectedMonitorData = useMemo(() => {
    return mockMonitors.find(m => m.id === selectedMonitor);
  }, [selectedMonitor]);

  // Generate performance metrics based on selected monitor
  const performanceMetrics = useMemo(() => {
    ...
    return {
      ...
      downtime: selectedMonitorData.status === 'down' ? 
        Math.floor(Math.random() * 60) : 0,
      longestDowntime: selectedMonitorData.status === 'down' ? 
        Math.floor(Math.random() * 30) : 0,
      incidentCount: selectedMonitorData.status === 'down' ? 
        Math.ceil(Math.random() * 5) : 0,
      avgResponseTime: selectedMonitorData.responseTime,
      indexApdex: selectedMonitorData.uptime >= 99.9 ? 1.0 : 
                  selectedMonitorData.uptime >= 99 ? 0.95 : 0.85,
    };
  }, [selectedMonitorData]);
  ...
  const handleCopyUrl = () => {
    if (selectedMonitorData) {
      ...
      toast({
        title: 'URL Copied',
        description: 'The monitor URL has been copied to clipboard.',
      });
    }
  };
  ...
  const handleSpinCheck = () => {
    toast({
      ...
      description: 'Running health check for ' + selectedMonitorData?.name,
    });
  };
  ...
  const handleViewAnalysis = (incidentId: string) => {
    toast({
      ...
      description: 'Viewing detailed analysis for incident #' + incidentId,
    });
  };
  ...
  return (
    <div className="min-h-screen">
      <Header 
        title="URL Performance Report" 
        subtitle="Detailed performance analytics and incident history"
      />

      <div className="p-6 space-y-6">
        {/* URL Selector and Controls */}
        <Card className="p-4">
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button onClick={handleSpinCheck} className="gap-2">
        ...
        {/* Historical Uptime Grid */}
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold">Historical uptime</h3>
            <span className="text-sm text-muted-foreground">
              Last 90 days — <span className="text-success">97.8%</span> avg. uptime
            </span>
          </div>
          <HistoricalUptimeGrid 
            monitorId={selectedMonitor}
            days={90}
          />
        </Card>
